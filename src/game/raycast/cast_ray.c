...
static void init_raycast_struct(game_data_t *d, size_t x, ray_t *ray)
{
    ...
    ray->perpendicular_dist = 0;
    ray->was_wall_hit = false;
    ray->was_x_side_hit = false;
}

void get_distance_1(ray_t *ray, game_data_t *d)
{
    if (ray->ray_dir.x < 0) {
        ray->step_dir.x = -1;
        ray->side_distance.x =
            (d->player.pos.x - (float) ray->map_tile.x) * ray->delta_distance.x;
    } else {
        ray->step_dir.x = 1;
...
void get_distance_2(ray_t *ray, game_data_t *d)
{
...
void get_wall_info(ray_t *ray, game_data_t *d)
{
    ...
    ray->wall_hit_coord = (ray->was_x_side_hit ?
        d->player.pos.y : d->player.pos.x) + ray->perpendicular_dist *
        (ray->was_x_side_hit ? ray->ray_dir.y : ray->ray_dir.x);
    ray->wall_hit_coord = modff(ray->wall_hit_coord, &ray->wall_hit_coord);
}

void draw_walls(ray_t *ray, game_data_t *d, size_t x)
{
