...
static void init_left(engine_t *engine, pause_menu_t *pause)
{
    ...
    pause->resume =
        create_btn(engine, "assets/sprites/main_menu/button_resume.png", &p1);
    pause->resume->on_click = &pause_on_resume;
    pause->resume->data = engine;
    pause->save =
        create_btn(engine, "assets/sprites/main_menu/button_exit.png", &p2);
    pause->save->on_click = &pause_on_save;
    pause->save->data = engine;
    pause->exit =
        create_btn(engine, "assets/sprites/main_menu/button_exit.png", &p3);
...
static void init_right(engine_t *engine, pause_menu_t *pause)
{
    ...
    pause->controls =
        create_btn(engine, "assets/sprites/main_menu/button_params.png", &p1);
